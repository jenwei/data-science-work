...
**Observations**:
  ...
  - How many aluminum alloys were tested? How do you know?
      - One - all rows have “al\_24st” listed as the alloy
  - What angles were tested?
      - 0, 45, 90
  - What thicknesses were tested?
      - 0.022, 0.032, 0.064, 0.081

Other questions - What’s the relationship across thickness, `E`, and
`mu`? - What’s the relationship across angle, `E`, and `mu`? - Are there
relationships across the data in the dataset? Are we missing data?
...
df_stang_long %>%
  ...
  coord_flip() +
  labs(title = "E by thickness", x = "thickness (in)")
```
...
**Observations**:
  ...
  - Overall, there is an inverse relationship between thickness and E -
    as E increases, thickness decreases
      - However, this relationship is not supported by the data when
        `thick = 0.064` as the median E is greater than that of `thick
        = 0.032`
...
df_stang_long %>%
  ...
  facet_grid(~ angle ~ .) +
  coord_flip() +
  labs(title = "E by thickness (facet by angle) ", x = "thickness (in)")
```
...
**Observations**:
  ...
  - Looking into the data more closely (i.e. by angle), the relationship
    between E and thickness becomes more questionable as the inverse
    relationship is supported by data when `angle = 45` but not when
    `angle = 0`
  - However, E for `thick = 0.022` seem consistent across all three
    angles
  - E for `thick = 0.081` also somewhat consistent across all three
    angles
      - Range of medians (and values in general) is much wider for
        `thick = 0.081` than `thick = 0.022` but is still distinct from
        other thicknesses
...
df_stang_long %>%
  ...
  coord_flip() +
  labs(title = "mu by thickness", x = "thickness (in)")
```
...
df_stang_long %>%
  ...
  facet_grid(~ angle ~ .) +
  coord_flip() +
  labs(title = "mu by thickness (facet by angle) ", x = "thickness (in)")
```
...
**Observations**:
  ...
  - Unlike E, mu data seems more chaotic and doesn’t seem to show a
    relationship between thickness and mu in general or by angle
  - Across the angles, there aren’t obvious patterns
