...
diamonds %>%
  ...
  geom_point() +
  ggtitle('`price` vs `carat` by `cut`')

diamonds %>%
  ggplot(aes(y = price, x = cut, color = carat)) +
  scale_colour_gradientn(colours=rainbow(4)) +
  geom_point(alpha = 1/10) +
  ggtitle('`price` vs `cut` by `carat`')
...
# TODO: [2020-07-10] Combine the two carat bar charts into one
diamonds %>%
  mutate(roundedCarat = round(carat)) %>%
...
## TODO: [2020-07-09] Figure out how to make carat (continuous) discrete
## diamonds %>%
##  mutate(roundedCarat = round(carat)) %>%
##  ggplot() +
##  geom_boxplot(aes(group = cut(roundedCarat(carat,))
##  geom_boxplot(aes(y = price, x = roundedCarat)) +
##  ggtitle('`price` vs `carat`')
```

**Observations**:
- There is an overall positive relationship between `price` and the other two variables (`carat` and `cut`)
- There is an overall negative relationship between `carat` and `cut` where the density of `Premium` and `Ideal` cuts are in the lower carats
- The density of data seems to show an almost exponential relationship between `price` and `carat` (between 0 and 2.5)
- `Fair` cuts tend to be the cheaper option for a given `carat`
- There are no `Ideal` cuts for diamonds greater than ~3.5 carats in this dataset
<!-- TODO: [2020-07-10] Add titles to graphs and figure names (if possible) -->
<!-- TODO: [2020-07-10] Add references to graphs in observations -->
